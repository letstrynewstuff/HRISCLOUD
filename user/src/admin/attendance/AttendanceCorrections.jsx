...
export default function AttendanceCorrections() {
  ...
  const openCorrection = (record) => {
    setSelected(record);
    setFormError("");
    const ci = record.clockIn
      ? new Date(record.clockIn).toTimeString().slice(0, 5)
      : "";
    const co = record.clockOut
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setRecords((prev) =>
        prev.map((r) =>
          r.id === selected.id ? (updated.record ?? { ...r, ...payload }) : r,
        ),
      );
      setSelected(null);
    } catch (err) {
      setFormError(
        err?.response?.data?.message ?? "Failed to save correction.",
      );
    } finally {
      setSaving(false);
    }
            ...
            >
              ...
              >
                ...
                <p className="text-xs" style={{ color: C.textMuted }}>
                  {selected.attendanceDate
                    ? new Date(selected.attendanceDate).toLocaleDateString(
                        "en-GB",
                        {
                          weekday: "long",
                          year: "numeric",
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, editReason: e.target.value }))
                  }
                  rows={3}
