...
// // Overtime records are attendance rows where overtime_hours > 0.
// // HR can view and correct them via the existing correct endpoint.
...
//       // Filter to records with overtime
...
//   // "Approve" overtime — we mark is_manually_edited as acknowledgment
//   // by updating the record with the same data + a reason note.
    ...
    >
      ...
      >
        {loading ? (
          ...
          <div className="py-12 flex flex-col items-center gap-2">
            <Clock size={28} style={{ color: C.textMuted }} />
            <p className="text-sm" style={{ color: C.textMuted }}>No overtime records found.</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr style={{ background: C.surfaceAlt, borderBottom: `1px solid ${C.border}` }}>
                  {["Employee", "Department", "Date", "Hours Worked", "Overtime Hours", "Status", "Actions"].map((h) => (
                ...
                {records.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-b" style={{ borderColor: C.border }}>
                      <td className="px-4 py-3 font-medium text-sm" style={{ color: C.textPrimary }}>{empName(r)}</td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 font-semibold text-sm" style={{ color: C.warning }}>
                        +{Number(r.overtimeHours).toFixed(1)}h
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-xs px-2.5 py-1 rounded-full font-semibold" style={{ background: approved ? "#D1FAE5" : "#FEF3C7", color: approved ? "#10B981" : "#F59E0B" }}>
                          {approved ? "Approved" : "Pending"}
                        </span>
                      </td>
                      <td className="px-4 py-3">
