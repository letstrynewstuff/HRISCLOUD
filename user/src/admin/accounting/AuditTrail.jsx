...
// //   const MODULES = ["all", "JournalEntry", "ChartOfAccounts"];
...
// //                         style={{ color: C.textMuted }}
// //                       >
// //                         {(log.recordId ?? log.record_id ?? "—").slice(0, 12)}…
// //                       </td>
// //                       <td className="px-4 py-3">
// //                         <span
// //                           className="text-xs font-semibold"
...
// //       <AnimatePresence>
// //         {selected && (
// //           <DiffDrawer log={selected} onClose={() => setSelected(null)} />
// //         )}
// //       </AnimatePresence>
// //     </Motion.div>
// //   );
...
//             {[
//               { label: "User", value: log.userName ?? log.user_name ?? log.userId ?? log.user_id ?? "—" },
//               { label: "Timestamp", value: log.timestamp ? new Date(log.timestamp).toLocaleString("en-NG") : "—" },
//               { label: "IP", value: log.ipAddress ?? log.ip_address ?? "—" },
//               { label: "Module", value: log.module },
//               { label: "Action", value: log.action },
//             ].map((d) => (
//               <div key={d.label} className="flex items-center justify-between text-xs">
//                 <span className="font-semibold" style={{ color: C.textMuted }}>{d.label}</span>
...
//                         {pVal != null ? (typeof pVal === 'object' ? JSON.stringify(pVal) : String(pVal)) : "—"}
//                       </p>
//                     </div>
//                     <div className="px-4 py-2.5 space-y-0.5">
//                       <p className="font-semibold" style={{ color: C.textMuted }}>{key}</p>
//                       <p className="font-mono break-all" style={{ color: "#10B981" }}>
//                         {nVal != null ? (typeof nVal === 'object' ? JSON.stringify(nVal) : String(nVal)) : "—"}
//                       </p>
//                     </div>
//                   </div>
//                 );
...
//             </div>
//           ) : (
//             <div className="rounded-2xl p-6 text-center text-sm" style={{ background: C.surface, border: `1px solid ${C.border}`, color: C.textMuted }}>
//               No value diff available for this entry.
//             </div>
//           )}
//         </div>
...
//   // 🟢 DYNAMIC MODULE AUTO-DISCOVERY + ACCOUNTING FALLBACKS
//   const MODULES = useMemo(() => {
//     const baseModules = ["all", "JournalEntry", "ChartOfAccounts", "TaxConfig", "VatReturn", "BankReconciliation", "ReconLineStatus"];
//     const activeLogModules = logs.map(l => l.module).filter(Boolean);
//     return Array.from(new Set([...baseModules, ...activeLogModules]));
//   }, [logs]);
...
//             {MODULES.map((m) => (
//               <option key={m} value={m}>
//                 {m === "all" ? "All Modules" : m.replace(/([A-Z])/g, ' $1').trim()}
//               </option>
//             ))}
//           </select>
//         </div>
...
//           <div className="flex flex-col items-center gap-3 py-16">
//             <Shield size={32} color={C.textMuted} />
//             <p className="text-sm font-medium" style={{ color: C.textPrimary }}>No audit logs found</p>
//             <p className="text-xs" style={{ color: C.textMuted }}>Try adjusting the filters or date range.</p>
//           </div>
//         ) : (
//           <div className="overflow-x-auto w-full">
//             <table className="w-full">
...
//                   {["Timestamp", "User", "Action", "Module", "Record ID", ""].map((h) => (
...
//                       </td>
//                       <td className="px-4 py-3">
//                         <span className="text-xs px-2.5 py-1 rounded-full font-bold" style={{ background: ac.bg, color: ac.color }}>
//                           {ac.label}
//                         </span>
//                       </td>
//                       <td className="px-4 py-3 text-sm" style={{ color: C.textSecondary }}>
//                         {log.module ? log.module.replace(/([A-Z])/g, ' $1').trim() : "—"}
//                       </td>
//                       <td className="px-4 py-3 font-mono text-xs" style={{ color: C.textMuted }}>
//                         {(log.recordId ?? log.record_id ?? "—").toString().slice(0, 12)}…
//                       </td>
//                       <td className="px-4 py-3">
//                         <span className="text-xs font-semibold" style={{ color: C.primary }}>View →</span>
//                       </td>
//                     </Motion.tr>
//                   );
//                 })}
