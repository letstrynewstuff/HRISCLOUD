...
import {
  Users, Search, Menu, Bell, ChevronRight, UserPlus, User,
  Edit3, Building2, Briefcase, GitBranch, Upload, LogOut,
  ClipboardCheck, UserCog, ArrowUpRight,
} from "lucide-react";
import { C, ADMIN } from "./sharedData";
import { getEmployees } from "../../api/service/employeeApi";

const TABS = [
  { id: "list",        label: "All Employees",       icon: Users,          path: "/admin/employeemanagement/admin-employeeslist",           exact: true, desc: "View, search and manage every employee record"          },
  { id: "new",         label: "Add Employee",         icon: UserPlus,       path: "/admin/employeemanagement/admin-addemployees",                         desc: "Onboard a new hire step-by-step"                        },
  { id: "profile",     label: "Employee Profile",     icon: User,           path: "/admin/employeemanagement/admin-viewemployeesprofile",                  desc: "View full employee details and history"                 },
  // { id: "edit",        label: "Edit Employee",        icon: Edit3,          path: "/admin/employeemanagement/admin-approvals",                            desc: "Update employee information with audit trail"           },
  // { id: "departments", label: "Departments",          icon: Building2,      path: "/admin/employeemanagement/admin-departments",                          desc: "Manage company departments and headcount"               },
  // { id: "roles",       label: "Job Roles",            icon: Briefcase,      path: "/admin/employeemanagement/admin-jobroles",                             desc: "Define and manage job roles and grades"                 },
  // { id: "org-chart",   label: "Org Chart",            icon: GitBranch,      path: "/admin/employeemanagement/admin-orgchart",                             desc: "Visualise the reporting hierarchy"                      },
  // { id: "bulk-import", label: "Bulk Import",          icon: Upload,         path: "/admin/employeemanagement/admin-import",                               desc: "Import multiple employees via CSV"                      },
  { id: "offboarding", label: "Offboarding",          icon: LogOut,         path: "/admin/employeemanagement/admin-offboarding",                          desc: "Manage employee exits and clearances"                   },
  // { id: "approvals",   label: "Approvals",            icon: ClipboardCheck, path: "/admin/employeemanagement/admin-approvals",                            desc: "Review and action pending HR approvals"                 },
  // { id: "changes",     label: "Change Requests",      icon: UserCog,        path: "/admin/employeemanagement/admin-profilechangerequests",                 desc: "Approve or reject employee profile update requests"     },
];

// ─── Real stats from API ──────────────────────────────────────
function useStats() {
  const [stats, setStats] = useState({
    total: 0, active: 0, onLeave: 0, inactive: 0, pending: 0,
  });
      ...
      .then((res) => {
        ...
        setStats(prev => ({
          ...
          inactive: employees.filter(e =>
            ["terminated", "resigned", "suspended", "retired", "offboarding"].includes(
              e.employment_status
            )
          ).length,
        }));
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          >
            ...
            <div className="flex items-center gap-1.5 text-xs" style={{ color: C.textSecondary }}>
              ...
              <span className="font-semibold" style={{ color: C.textPrimary }}>Employee Management</span>
              {activeTabId !== "list" && (
                <>
                  <ChevronRight size={11} />
                  <span className="font-semibold" style={{ color: C.primary }}>{activeTab.label}</span>
                </>
              )}
            </div>
          ...
          <main className="flex-1 overflow-y-auto p-5 md:p-7 space-y-6">
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: C.primaryLight }}>
                  <activeTab.icon size={16} color={C.primary} />
                </div>
                <div>
                  <p className="text-sm font-bold" style={{ color: C.textPrimary }}>{activeTab.label}</p>
                  <p className="text-xs mt-0.5" style={{ color: C.textSecondary }}>{activeTab.desc}</p>
                </div>
              </div>
                  ...
                  return (
                    <motion.button
                      ...
                      style={{
                        ...
                        boxShadow: active ? `0 4px 16px rgba(79,70,229,0.14)` : "0 2px 8px rgba(15,23,42,0.04)",
                        cursor: "pointer",
                    ...
                    >
                      <div
                        ...
                        style={{ background: active ? C.primary : C.primaryLight }}
                      >
                        <Icon size={16} color={active ? "#ffffff" : C.primary} />
                      </div>
                      <div>
                        <p className="text-xs font-bold leading-tight" style={{ color: active ? C.primary : C.textPrimary }}>
                          {tab.label}
                        </p>
                        <p className="text-[10px] mt-1 leading-snug" style={{ color: C.textMuted }}>
                          {tab.desc}
                        </p>
                      </div>
                    </motion.button>
