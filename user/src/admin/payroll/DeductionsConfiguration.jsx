...
// Statutory deductions are compulsory by law and already rendered above
// with a fixed "Always Active" badge. If the backend/custom-deductions
// table ever contains a row named "PAYE...", "NHF...", or "Pension...",
// it's a duplicate placeholder — not a real custom deduction — so it's
// filtered out of the editable list below rather than shown as a
// togglable/deletable row that could be switched to "Inactive".
const STATUTORY_NAME_PATTERN = /paye|nhf|pension/i;
...
export default function DeductionsConfiguration() {
  ...
  const fetchDeductions = useCallback(async () => {
    ...
    try {
      ...
      const customOnly = (res.data ?? []).filter(
        (d) => !STATUTORY_NAME_PATTERN.test(d.name ?? ""),
      );
      setDeductions(customOnly);
    } catch (e) {
      setError(e?.response?.data?.message ?? "Failed to load deductions.");
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    const timeoutId = setTimeout(fetchDeductions, 0);
  ...
  const handleSave = async () => {
    const errs = {};
    if (!form.name.trim()) errs.name = "Required";
    else if (STATUTORY_NAME_PATTERN.test(form.name.trim())) {
      errs.name =
        "This is a statutory deduction and is already handled automatically above — it can't be added as a custom deduction.";
    }
    if (!form.value) errs.value = "Required";
    else if (isNaN(Number(form.value))) errs.value = "Must be a number";
    setFormErr(errs);
  ...
  return (
    <div className="space-y-8">
      {/* Statutory — fixed, informational, always active, not editable */}
      <div>
        <h2
          className="font-semibold text-lg mb-4"
          style={{ color: C.textPrimary }}
