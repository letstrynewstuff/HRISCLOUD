...
//  8. Step 3 approval gate — three checkboxes (CEO, Finance, HR review) must
//     all be ticked before "Finalise" is enabled. UI-only; no backend changes.
...
function FormatPicker({ value, onChange }) {
  const options = [
    { id: "csv", icon: Table2,   label: "CSV",       sub: "Bank portal upload" },
    { id: "pdf", icon: FileText, label: "PDF / HTML", sub: "Full payroll report" },
  ];
        ...
        {options.map((f) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <p className="text-sm font-bold leading-tight" style={{ color: active ? C.primary : C.textPrimary }}>
                  {f.label}
                </p>
                <p className="text-[10px] mt-0.5" style={{ color: C.textMuted }}>{f.sub}</p>
              </div>
            </button>
          );
        })}
...
function ApprovalGate({ checked, onChange }) {
  ...
  return (
    <div
      ...
      style={{
        borderColor: allChecked ? C.success : C.border,
    ...
    >
      ...
      <div
        ...
        style={{
          background:   allChecked ? C.successLight : C.surfaceAlt,
          borderBottom: `1px solid ${allChecked ? `${C.success}33` : C.border}`,
          transition:   "background 0.3s",
        }}
      >
        <div
          className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
          style={{ background: allChecked ? C.success : C.border }}
        >
          {allChecked
        ...
        <div className="flex-1">
          <p className="font-bold text-sm" style={{ color: allChecked ? C.success : C.textPrimary }}>
            {allChecked ? "All approvals confirmed — ready to finalise" : "Approval confirmation required"}
          </p>
          <p className="text-xs mt-0.5" style={{ color: C.textMuted }}>
            Tick all three boxes to confirm the necessary parties have authorised this payroll.
          </p>
        </div>
      ...
      <div className="divide-y" style={{ borderColor: C.border }}>
        {APPROVAL_ITEMS.map((item) => {
          ...
          return (
            <motion.label
              key={item.id}
              whileHover={{ backgroundColor: `${C.primaryLight}60` }}
            ...
            >
              ...
              <div className="flex-1" onClick={() => onChange(item.id, !isChecked)}>
                <p
                  className="text-sm font-semibold leading-tight"
                  style={{ color: isChecked ? C.success : C.textPrimary }}
                >
                  {item.label}
                </p>
      ...
      {!allChecked && (
        ...
        >
          {APPROVAL_ITEMS.filter((i) => !checked[i.id]).length} confirmation{APPROVAL_ITEMS.filter((i) => !checked[i.id]).length > 1 ? "s" : ""} still needed before you can finalise.
        </div>
      )}
    </div>
  );
...
export default function RunPayroll({ onComplete }) {
  ...
  // Approval gate state — all three must be ticked
  const [approvals, setApprovals] = useState({ ceo: false, finance: false, hr: false });
                    ...
                    <div key={i} className="flex justify-between items-center px-6 py-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span
                          ...
                          style={{
                            background: (ded.isStatutory ?? ded.is_statutory) ? C.primaryLight : C.successLight,
        ...
        {currentStep === 3 && (
          ...
          >
            <div>
              ...
              <p className="text-sm" style={{ color: C.textMuted }}>
                Download the payroll summary, get sign-off from the required parties, then finalise.
              </p>
            </div>
            ...
            {/* ── Download summary for approvers ── */}
            <div
            ...
            >
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs mt-0.5" style={{ color: C.textMuted }}>
                    Share this file with the CEO, Finance, and HR teams so they can review totals before signing off.
                  </p>
                </div>
              </div>
              <FormatPicker value={exportFormat} onChange={setExportFormat} />
              ...
              >
                ...
                Download {exportFormat === "pdf" ? "PDF Report" : "Bank Transfer CSV"} for Review
              </motion.button>
            </div>
            ...
            <ApprovalGate
              checked={approvals}
              onChange={(id, val) => setApprovals((prev) => ({ ...prev, [id]: val }))}
            />

            {/* Mode-specific next steps note */}
            {selectedMode === "assisted" ? (
              <div
                className="p-4 rounded-xl text-xs space-y-1"
                style={{ background: C.primaryLight, border: `1px solid ${C.primary}22`, color: C.textSecondary }}
              >
                <p className="font-semibold" style={{ color: C.primary }}>After finalising:</p>
                <p>For CSV — log into your bank portal (Access, GTBank, UBA…) and upload the transfer file.</p>
                <p>Return and click "Mark as Paid" after the bank confirms all transfers.</p>
              </div>
            ) : (
              <div
                className="p-4 rounded-xl text-xs space-y-1"
              ...
              >
                <p className="font-semibold" style={{ color: C.textPrimary }}>After finalising:</p>
                <p>Transfer salaries individually via your bank, then return and click "Mark as Paid".</p>
              </div>
            )}

            {/* Finalise button — locked until all approvals ticked */}
            <motion.button
              whileHover={allApproved ? { scale: 1.02 } : {}}
              ...
              style={{
                ...
                border:     "none",
                cursor:     loading || !allApproved ? "not-allowed" : "pointer",
            ...
            >
              {loading ? (
                ...
                <><Lock size={16} /> Confirm all approvals to finalise</>
              )}
            </motion.button>
          </motion.div>
        )}
