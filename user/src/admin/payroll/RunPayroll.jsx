...
//             className="mt-6 flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold mx-auto"
//             style={{
...
//           }}
//           className="mt-3 px-6 py-2.5 rounded-xl text-sm font-semibold mx-auto flex"
//           style={{
...
//                   Payroll Run <ArrowRight size={18} />
//                 </>
//               )}
//             </motion.button>
//           </motion.div>
...
//               </h2>
//               <p className="text-sm" style={{ color: C.textMuted }}>
//                 System will calculate all active employees. Click Process to
//                 compute.
//               </p>
//             </div>
...
//                   >
//                     Run Initiated — {MONTHS[period.month - 1]} {period.year}
//                   </p>
//                   <p className="text-xs" style={{ color: C.textMuted }}>
...
//                         {["Employee", "Basic", "Allowances", "Gross"].map(
//                           (h) => (
//                             <th
//                               key={h}
...
//               </h2>
//               <p className="text-sm" style={{ color: C.textMuted }}>
//                 Active deductions applied to this run. Manage in the Deductions
//                 tab.
//               </p>
//             </div>
...
//               </h2>
//               <p className="text-sm" style={{ color: C.textMuted }}>
//                 Review totals before finalising. This action generates all
//                 payslips.
//               </p>
//             </div>
...
//                 {
//                   label: "Gross Payroll",
//                   value: fmtM(totalGross),
//                   color: C.primary,
//                   bg: C.primaryLight,
//                 },
//                 {
//                   label: "Total Deductions",
//                   value: fmtM(totalDeductions),
//                   color: C.danger,
//                   bg: C.dangerLight,
//                 },
//                 {
...
//                   <li>
//                     5. Return here and click "Mark as Paid" once bank confirms.
//                   </li>
//                 </ol>
//               </div>
//             ) : (
...
//                   <li>
//                     3. Click "Mark as Paid" once all transfers are complete.
//                   </li>
//                 </ol>
//               </div>
//             )}
...
// //
// // FIXES applied:
// //  1. handleInit — recovers gracefully from 409 (existing run) instead of crashing.
// //     Reads run ID from the 409 body and continues to step 1.
// //  2. handleProcess — getRun returns { run, records } not { data.items }.
// //     Maps records using the correct payroll_records field names from the DB.
// //  3. handleDownloadCSV — calls the backend CSV endpoint with responseType blob
// //     and triggers a real browser file download.
// //  4. Summary totals — now read from activeRun (set after process) so they
// //     show correct figures even before step 3.
...
//         <div
//           className="w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5"
//           style={{ borderColor: selected ? C.primary : C.border, background: selected ? C.primary : "transparent" }}
//         >
//           {selected && <div className="w-2 h-2 rounded-full bg-white" />}
//         </div>
//       </div>
...
//   const [notes,         setNotes]         = useState("");
//   const [activeRun,     setActiveRun]     = useState(null);
//   // runItems — array of payroll_records rows (field names match DB / fmtRecord)
//   const [runItems,      setRunItems]      = useState([]);
//   const [deductions,    setDeductions]    = useState([]);
//   const [loading,       setLoading]       = useState(false);
//   const [error,         setError]         = useState(null);
...
//   // Derive totals from runItems using the real DB field names (fmtRecord camelCase)
...
//       // FIX 1: 409 means a run for this month already exists.
//       // The backend returns { message, data: { id, status, ... } }.
//       // Recover the existing run and move straight to step 1.
...
//   // ── STEP 1 → process ──────────────────────────────────────
//   const handleProcess = async () => {
//     if (!activeRun?.id) { setError("No active run — please go back to step 1."); return; }
//     setLoading(true);
//     setError(null);
//     try {
//       // runProcess calls POST /runs/:id/process
//       await runProcess(activeRun.id);

//       // FIX 2: getRun returns { run, records, total } — NOT { data: { items } }
//       const full = await getRun(activeRun.id);
//       const run  = full.run ?? full.data ?? full;
//       const recs = full.records ?? [];

//       setActiveRun(run);
//       setRunItems(recs);   // records use fmtRecord camelCase keys
//       setCurrentStep(2);
//     } catch (e) {
//       setError(e?.response?.data?.message ?? "Processing failed.");
//     } finally {
...
//   // ── CSV download ──────────────────────────────────────────
//   // FIX 3: getPaymentFile must return a Blob. See payrollApi note below.
//   const handleDownloadCSV = async () => {
//     if (!activeRun?.id) { setError("No active run to export."); return; }
//     setDownloadingCSV(true);
//     setError(null);
//     try {
//       // const blob = await getPaymentFile(activeRun.id);
//       const blob = await getPaymentFile(activeRun.id, "csv", "pdf"); // or "pdf"
//       const url = URL.createObjectURL(blob);
//       const a = document.createElement("a");
//       a.href = url;
//       a.download = `payroll-transfer-${period.year}-${String(period.month).padStart(2, "0")}.csv`;
...
//       URL.revokeObjectURL(url);
//     } catch (e) {
//       setError("Failed to download payment file. Ensure the run has been processed first.");
//     } finally {
//       setDownloadingCSV(false);
//     }
//   };
...
//           <CheckCircle2 size={44} color={C.success} />
//         </div>
//         <h2 className="text-2xl font-bold mb-2" style={{ color: C.textPrimary, fontFamily: "Sora,sans-serif" }}>
//           Payroll Finalised!
//         </h2>
//         <p className="text-sm mb-1" style={{ color: C.textSecondary }}>
//           {MONTHS[period.month - 1]} {period.year} payroll —{" "}
//           {selectedMode === "assisted" ? "payment file ready for bank upload" : "marked as paid"}.
//         </p>
//         <p className="text-xs" style={{ color: C.textMuted }}>
//           {runItems.length} payslips generated · Net pay: {fmtM(totalNet)}
//         </p>

//         {/* CSV download always available on success for assisted mode */}
//         {selectedMode === "assisted" && (
//           <motion.button
//             whileHover={{ scale: 1.02 }}
//             onClick={handleDownloadCSV}
//             disabled={downloadingCSV}
//             className="mt-6 flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold mx-auto"
//             style={{ background: C.primary, color: "#fff", border: "none", cursor: "pointer" }}
//           >
...
//           whileHover={{ scale: 1.02 }}
//           onClick={() => {
//             setCurrentStep(0); setActiveRun(null); setRunItems([]); setShowSuccess(false);
//             if (onComplete) onComplete();
//           }}
//           className="mt-3 px-6 py-2.5 rounded-xl text-sm font-semibold mx-auto flex"
...
//             <button onClick={() => setError(null)} style={{ background: "none", border: "none", cursor: "pointer", color: C.danger }}>✕</button>
//           </motion.div>
//         )}
//       </AnimatePresence>
...
//             <div className="space-y-3">
//               <ModeCard
//                 icon={ClipboardList} title="Manual Payroll"
//                 description="System calculates salaries. You pay staff directly via your bank. No file export needed."
//                 selected={selectedMode === "manual"}
//                 onClick={() => setSelectedMode("manual")}
//               />
//               <ModeCard
//                 icon={Zap} title="Assisted Payroll" badge="Recommended"
//                 description="System calculates payroll and generates a CSV bank transfer list. Upload it to your bank portal (Access, GTBank, UBA, etc.)."
//                 selected={selectedMode === "assisted"}
//                 onClick={() => setSelectedMode("assisted")}
//               />
//             </div>
...
//               <p className="text-sm" style={{ color: C.textMuted }}>Click "Process Payroll" to calculate all active employees.</p>
//             </div>
...
//                     Run {activeRun?.status === "draft" ? "Ready" : activeRun?.status ?? "Initiated"} — {MONTHS[period.month - 1]} {period.year}
//                   </p>
//                   <p className="text-xs" style={{ color: C.textMuted }}>
//                     Mode: {selectedMode} · Run ID: {activeRun?.id?.slice(0, 8)}…
//                   </p>
...
//             {/* Employee earnings table — populated after processing */}
//             {runItems.length > 0 ? (
...
//                           <td className="px-5 py-3.5 font-medium text-sm" style={{ color: C.textPrimary }}>
//                             {emp.employeeName ?? emp.employee_name ?? emp.employeeId ?? "—"}
//                           </td>
//                           <td className="px-5 py-3.5 text-sm">{fmt(emp.basicSalary    ?? emp.basic_salary)}</td>
//                           <td className="px-5 py-3.5 text-sm">{fmt(emp.housingAllowance ?? emp.housing_allowance)}</td>
//                           <td className="px-5 py-3.5 text-sm">{fmt(emp.transportAllowance ?? emp.transport_allowance)}</td>
//                           <td className="px-5 py-3.5 font-semibold text-sm" style={{ color: C.primary }}>
...
//                 </div>
//                 {runItems.length > 10 && (
//                   <p className="px-5 py-3 text-xs" style={{ color: C.textMuted }}>…and {runItems.length - 10} more employees</p>
//                 )}
//               </div>
//             ) : (
...
//                 <p className="px-6 py-8 text-sm text-center" style={{ color: C.textMuted }}>No active deductions configured.</p>
//               ) : (
//                 <div className="divide-y" style={{ borderColor: C.border }}>
//                   {deductions.filter(d => d.isActive ?? d.is_active).map((ded, i) => (
...
//                           {ded.type === "percent" ? `${ded.value}%` : ded.type === "formula" ? "Auto" : fmt(ded.value)}
//                         </span>
//                         <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full"
//                           style={{ background: ded.isStatutory ?? ded.is_statutory ? C.primaryLight : C.successLight,
//                                    color:      ded.isStatutory ?? ded.is_statutory ? C.primary      : C.success }}>
...
//                           </td>
//                           <td className="px-4 py-3 text-sm">{fmt(emp.grossSalary    ?? emp.gross_salary)}</td>
//                           <td className="px-4 py-3 text-sm">{fmt(emp.payeTax        ?? emp.paye_tax)}</td>
//                           <td className="px-4 py-3 text-sm">{fmt(emp.pensionEmployee ?? emp.pension_employee)}</td>
//                           <td className="px-4 py-3 text-sm">{fmt(emp.nhfDeduction   ?? emp.nhf_deduction)}</td>
//                           <td className="px-4 py-3 text-sm">{fmt(emp.otherDeductions ?? emp.other_deductions)}</td>
//                           <td className="px-4 py-3 text-sm font-bold" style={{ color: C.success }}>
...
//                 Payroll Summary & Approval
//               </h2>
//               <p className="text-sm" style={{ color: C.textMuted }}>Review totals before finalising. This generates all payslips.</p>
//             </div>

//             {/* Totals — use activeRun figures (set from the DB response) with fallback to computed */}
//             <div className="grid grid-cols-3 gap-4">
//               {[
//                 { label: "Gross Payroll",    value: fmtM(activeRun?.totalGross      ?? totalGross),      color: C.primary, bg: C.primaryLight },
//                 { label: "Total Deductions", value: fmtM(activeRun?.totalDeductions ?? totalDeductions), color: C.danger,  bg: C.dangerLight  },
//                 { label: "Net Pay",          value: fmtM(activeRun?.totalNet        ?? totalNet),        color: C.success, bg: C.successLight },
//               ].map(({ label, value, color, bg }) => (
...
//               {selectedMode === "assisted" && (
//                 <motion.button whileHover={{ scale: 1.02 }}
//                   onClick={handleDownloadCSV} disabled={downloadingCSV}
//                   className="w-full py-3.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2"
//                   style={{ background: C.surface, color: C.primary, border: `2px solid ${C.primary}`, cursor: "pointer" }}
//                 >
//                   {downloadingCSV ? <Loader2 size={14} className="animate-spin" /> : <Download size={14} />}
//                   Download Bank Transfer CSV
//                 </motion.button>
...
//                 className="w-full py-4 rounded-xl text-base font-bold text-white flex items-center justify-center gap-2"
//                 style={{ background: C.success, border: "none", cursor: loading ? "not-allowed" : "pointer" }}
//               >
//                 {loading
