...
export default function MobileBottomNav({ setSidebarOpen }) {
  ...
  return (
    <>
      {/* Spacer — only on < lg */}
      <div className="lg:hidden shrink-0" style={{ height: 64 }} aria-hidden="true" />
        ...
        {tabs.map((tab, i) => {
          ...
          return (
            <Motion.button
              ...
              onClick={tab.action ?? (() => navigate(tab.path))}
              className="flex-1 flex flex-col items-center justify-center gap-1 relative"
              style={{ background: "none", border: "none", outline: "none", cursor: "pointer", WebkitTapHighlightColor: "transparent" }}
            >
              <AnimatePresence>
                {isActive && (
                  <Motion.div
              ...
              <div
                className="flex items-center justify-center rounded-2xl transition-all duration-200"
                style={{ width: 42, height: 30, background: isActive ? C.primaryLight : "transparent" }}
              >
                <Icon size={20} strokeWidth={isActive ? 2.5 : 1.8} color={isActive ? C.primary : "#94A3B8"} />
              </div>
