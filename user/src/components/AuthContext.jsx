...
export function AuthProvider({ children }) {
  ...
  const fetchMe = useCallback(async () => {
    ...
    try {
      ...
      // ─────────────────────────────────────────────
      if (!token) {
        // Public pages do not need authentication.
        if (isPublicPage) {
          setUser(null);
          setEmployee(null);
          setError(null);
        ...
        // Protected page: try to restore the session
        // using the refresh cookie.
        const refresh = await authApi.refresh();

        setCookie("accessToken", refresh.accessToken, 60);
      }

      // ─────────────────────────────────────────────
      // We have authentication, fetch the user
      // ─────────────────────────────────────────────
      const res = await authApi.getMe();
      const u = res.user ?? res.data ?? res;
      ...
      // Only redirect to login when the current page
      ...
      if (!isPublicPage) {
        ...
        // Public pages should remain accessible even when
