...
//   • "Chat" button on each member card now navigates to /chat with
//     state { openDM: employee.id } so Chat.jsx auto-opens that DM.
//   • Removed the "Leave Team" button — team membership is manager-controlled.
//   • Manager sees an "Add/Remove Members" note linking to chat channel.
...
//   Menu, Bell, Search, MessageSquare, ChevronRight,
...
//   const colors = { active: C.success, onleave: C.warning, offline: C.textMuted };
//   return <div className="w-2.5 h-2.5 rounded-full" style={{ background: colors[status] ?? C.textMuted }} />;
// }
...
//       );
//       const peers   = allEmps.filter((e) => e.id !== myId && e.id !== manager?.id);
...
//   useEffect(() => { fetchData(); }, [employee?.departmentId]);
...
//   if (error) {
//     return (
//       <div className="flex flex-col items-center justify-center h-screen gap-3" style={{ background: C.bg }}>
//         <AlertCircle size={36} color={C.danger} />
//         <p className="font-semibold" style={{ color: C.textPrimary }}>{error}</p>
...
//               </motion.button>
//               <motion.button className="relative p-2 rounded-xl"
...
//                 {EMPLOYEE.department || "Your Department"} · {teamMembers.length + (lineManager ? 1 : 0)} members
//               </p>
//             </motion.div>
...
//                           {channels.filter((c) => c.isActive).length} active team channel{channels.filter((c) => c.isActive).length !== 1 ? "s" : ""}
//                         </p>
...
//                     className="rounded-2xl bg-white p-6 border shadow-sm" style={{ borderColor: C.border }}>
...
//                         {lineManager.first_name?.[0]}{lineManager.last_name?.[0]}
//                       </div>
//                       <div className="flex-1 min-w-0">
//                         <p className="font-semibold text-xl">{lineManager.first_name} {lineManager.last_name}</p>
//                         <p style={{ color: C.textSecondary }}>{lineManager.job_role_name}</p>
//                         {lineManager.email && (
//                           <p className="text-sm mt-0.5" style={{ color: C.textMuted }}>{lineManager.email}</p>
//                         )}
//                       </div>
...
//                       {filteredTeam.length} colleague{filteredTeam.length !== 1 ? "s" : ""}
//                     </span>
//                   </div>

//                   {filteredTeam.length === 0 ? (
//                     <div className="rounded-2xl bg-white p-12 flex flex-col items-center gap-3 border"
//                       style={{ borderColor: C.border }}>
//                       <Users size={36} color={C.textMuted} />
//                       <p className="font-semibold" style={{ color: C.textPrimary }}>No colleagues found</p>
//                       <p className="text-sm" style={{ color: C.textMuted }}>
//                         {searchQuery ? "Try a different search term." : "Your team data is being set up."}
//                       </p>
//                     </div>
//                   ) : (
...
//                                 style={{ background: `linear-gradient(135deg,hsl(${i * 47 + 200},65%,48%),hsl(${i * 47 + 240},65%,52%))` }}>
//                                 {initials}
//                               </div>
//                               <div className="flex-1 min-w-0">
//                                 <p className="font-semibold truncate">{name}</p>
//                                 <p className="text-sm truncate" style={{ color: C.textSecondary }}>
//                                   {member.job_role_name ?? member.role ?? "Employee"}
//                                 </p>
//                               </div>
//                               <StatusDot status={member.on_leave ? "onleave" : "active"} />
//                             </div>
...
//                               </motion.button>
//                               {member.email && (
//                                 <motion.a whileHover={{ scale: 1.02 }}
...
//                               Returns {new Date(person.return_date).toLocaleDateString("en-NG", { month: "short", day: "numeric" })}
//                             </p>
//                           )}
//                         </div>
//                       ))}
...
import {
  Users, Mail, Phone, Calendar, Clock,
  Menu, Bell, Search, MessageSquare, ChevronRight,
  Loader2, AlertCircle, RefreshCw, MessageCircle, Plus
} from "lucide-react";
import { getEmployees } from "../api/service/employeeApi";
import { chatApi }      from "../api/service/chatApi";
import { useAuth }      from "../components/AuthContext";
...
export default function TeamPage() {
  ...
  // ── Permission Check ───────────────────────────────────────────────────────
  const canCreateGroup = ["manager", "hr_admin", "super_admin"].includes(user?.role) || employee?.is_manager;
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <header className="shrink-0 h-[60px] flex items-center px-5 gap-4 z-10"
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              {canCreateGroup && (
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                  onClick={() => navigate("/chat", { state: { openCreateChannel: true } })}
