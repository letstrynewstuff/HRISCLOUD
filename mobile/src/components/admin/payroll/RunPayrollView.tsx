...
export default function RunPayrollView({ onClose }: Props) {
  ...
  // ── STEP 2 → approve ──
  const checkAnomalies = async (runId: string) => {
    console.log("checkAnomalies called with runId:", runId); // TEMP
    setAiChecking(true);
    setAnomalyResult(null);
    try {
      const res = await getPayrollAnomalies(runId);
      console.log("anomalies result:", res); // TEMP
      ...
      setAnomalyResult({
        error: true,
        message: "AI check failed — you can still review manually.",
      });
    } finally {
      setAiChecking(false);
    }
          ...
          <View style={{ gap: 16 }}>
            ...
            >
              ...
              >
                ...
                <Text style={styles.cardTitle}>AI Payroll Check</Text>
              </View>
              ...
              {aiChecking && (
                ...
                >
                  ...
                  <Text style={styles.muted}>
                    Checking calculations for errors…
                  </Text>
                </View>
              )}

              {!aiChecking && anomalyResult && !anomalyResult.error && (
                <View style={{ marginTop: 10 }}>
                  <Text
                    style={{
                      ...
                      fontWeight: "700",
                      color:
                        anomalyResult.anomalyCount > 0 ? C.danger : C.success,
                    }}
                  >
                    {anomalyResult.anomalyCount > 0
                      ? `⚠ ${anomalyResult.anomalyCount} issue(s) found`
                      : "✓ No issues found — all calculations verified"}
                  </Text>
                  <Text
                    style={[styles.muted, { marginTop: 6, lineHeight: 17 }]}
                  >
              ...
              {!aiChecking && anomalyResult?.error && (
