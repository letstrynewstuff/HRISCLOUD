...
const VALID_SCHEDULE_TYPES = ["fixed", "hours_target"];
...
// ── Shared schedule-type field rules (used by both create + update) ──
// NOTE: whether startTime/endTime or targetHoursPerDay are actually
// *required* depends on which scheduleType ends up in effect (and, on
// update, whatever the existing shift already had) — that branching
// lives in the controller (createShift/updateShift), not here. These
// rules only check format/range when a field is present.
const scheduleTypeRule = () =>
  body("scheduleType")
    .optional({ nullable: true, checkFalsy: true })
    .trim()
    .isIn(VALID_SCHEDULE_TYPES)
    .withMessage(
      `scheduleType must be one of: ${VALID_SCHEDULE_TYPES.join(", ")}.`,
    );

const targetHoursPerDayRule = () =>
  body("targetHoursPerDay")
    .optional({ nullable: true, checkFalsy: true })
    .isFloat({ min: 0.5, max: 24 })
    .withMessage("targetHoursPerDay must be between 0.5 and 24.");

const graceMinutesBeforeRule = () =>
  body("graceMinutesBefore")
    .optional({ nullable: true, checkFalsy: true })
    .isInt({ min: 0, max: 180 })
    .withMessage("graceMinutesBefore must be between 0 and 180.");

const lateGraceMinutesRule = () =>
  body("lateGraceMinutes")
    .optional({ nullable: true, checkFalsy: true })
    .isInt({ min: 0, max: 180 })
    .withMessage("lateGraceMinutes must be between 0 and 180.");
...
export const createShiftRules = [
  ...
  // Not marked required here — the controller enforces startTime/endTime
  // for 'fixed' shifts and targetHoursPerDay for 'hours_target' shifts,
  // since which one is required depends on scheduleType.
  timeRule("startTime"),
  timeRule("endTime"),
  targetHoursPerDayRule(),
  graceMinutesBeforeRule(),
  lateGraceMinutesRule(),
