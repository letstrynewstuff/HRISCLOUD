...
/**
 * Formats a goal row from the database into a clean JSON object.
 * Removed references to the non-existent 'achieved_value' column.
 */
function fmt(g) {
  return {
    id: g.id,
...
export async function assignGoal(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      message: `Goal assigned to ${assigned.length} employee(s).`,
      assigned,
      skipped,
    });
  } catch (err) {
...
// ✅ FIXED: Removed duplicate 'progress' assignment
// ✅ FIXED: Removed non-existent 'achievedValue' mapping
export async function updateGoalProgress(req, res) {
  const { id } = req.params;
  const { companyId, userId, role } = req.user;
