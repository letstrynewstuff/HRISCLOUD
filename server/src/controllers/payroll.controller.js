...
function fmtRecord(r) {
  return {
    ...
    payslipGenerated: r.payslip_generated,
    // ── added ──
    runStatus: r.run_status, // from: prun.status AS run_status
    accountNumber: r.account_number, // from: e.account_number (if joined)
    bankName: r.bank_name, // from: e.bank_name (if joined)
    paymentDate: r.paid_at, // from: prun.paid_at (if joined)
  };
}
...
export async function getMyPayslip(req, res) {
  ...
  try {
    ...
    req.params.employeeId = emp.rows[0].id.toString(); 
