...
// SHIFTS
//
// FIX: createShift/updateShift previously only wrote start_time,
//      end_time, and days — schedule_type, target_hours_per_day,
//      grace_minutes_before, and late_grace_minutes existed on the
//      table (migration 20260704) but were never referenced here,
//      so an "hours-based" shift saved from the UI would silently
//      persist as a plain fixed shift with those columns left at
//      their defaults.
//
//      The controller now resolves the full set of schedule fields
//      before calling these (nulling out whichever side — fixed vs
//      hours_target — doesn't apply), so both functions write them
//      directly rather than COALESCE-ing against the old row.
// ─────────────────────────────────────────────────────────────
export const createShift = async (companyId, data) => {
  const result = await db.query(
    `INSERT INTO shifts (
       company_id, name, description,
       schedule_type, start_time, end_time, days,
       target_hours_per_day, grace_minutes_before, late_grace_minutes
     )
    ...
    [
      ...
      data.endTime ?? null,
      data.days,
      data.targetHoursPerDay ?? null,
      data.graceMinutesBefore ?? null,
...
export const updateShift = async (shiftId, companyId, data) => {
  const result = await db.query(
    ...
    [
      ...
      data.days ?? null,
      data.isActive ?? null,
      data.targetHoursPerDay,
      data.graceMinutesBefore,
...
export const getShifts = async (companyId) => {
  const result = await db.query(
    `SELECT * FROM shifts WHERE company_id = $1 ORDER BY start_time ASC NULLS LAST, name ASC`,
    [companyId],
  );
  return result.rows;
};
